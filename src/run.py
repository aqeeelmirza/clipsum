...
if __name__ == '__main__':
    ...
    parser.add_argument('-visual_hidden_size', type=int, default=512,
                       help='Visual feature dimension: 512 for CLIP, 2048 for ResNet')
    parser.add_argument('-clip_model', type=str, default='openai/clip-vit-base-patch32',
                       help='CLIP model name for end-to-end training (Phase 2)')
    parser.add_argument('-freeze_clip', action='store_true',
                       help='Freeze CLIP weights during training (for end-to-end CLIP)')
    parser.add_argument('-video_dir', type=str, default='./dataset/videos',
                       help='Directory containing video files (for end-to-end CLIP training)')
    parser.add_argument('-num_frames', type=int, default=50,
                       help='Number of frames to sample per video (for end-to-end CLIP)')
    ...
    if args.do_train == 'True':
        trainer.fit(model, summary_data, ckpt_path=args.checkpoint)
    if args.do_test == 'True':
        model = BartMultiModal.load_from_checkpoint(args.checkpoint, args=args)
