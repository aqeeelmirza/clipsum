...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None,
                 fusion_layer=None, use_img_trans=None, use_forget_gate=None, cross_attn_type=None, dim_common=256, n_attn_heads=1, visual_hidden_size=2048):
        ...
        # NEW: Store visual_hidden_size parameter
        self.visual_hidden_size = visual_hidden_size

        if self.use_img_trans:
            self.img_transformer = ImageTransformerEncoder(d_model=self.visual_hidden_size, num_layers=4, num_heads=8, dim_feedforward=self.visual_hidden_size)

        # Some global variables
        visual_feature_dim = self.visual_hidden_size  # MODIFIED: Was hardcoded 2048
        ...
        # # # ============================ image transformer ==========================================
        # # self.img_transformer = ImageTransformerEncoder(d_model=self.visual_hidden_size, num_layers=3, num_heads=8, dim_feedforward=self.visual_hidden_size)
        # # # ============================ image transformer ==========================================
        # # ======================== global attention ========================
        # img_feature_dim = 2048
        # text_feature_dim = 768
        # dim_common = 768
        # self.img_feature_transfer = nn.Linear(img_feature_dim, dim_common, bias=True)
        # # self.text_feature_transfer = nn.Linear(text_feature_dim, dim_common, bias=True)
        # self.fc = nn.Linear(img_feature_dim + text_feature_dim, text_feature_dim, bias=True)
        # self.img_feature_transfers = nn.ModuleList([self.img_feature_transfer for _ in range(config.encoder_layers)])
        # # self.text_feature_transfers = nn.ModuleList([self.text_feature_transfer for _ in range(config.encoder_layers)])
        # self.fg = nn.Linear(img_feature_dim+text_feature_dim, img_feature_dim, bias=True)
        # self.fcs = nn.ModuleList([self.fc for _ in range(config.encoder_layers)])
        # self.fgs = nn.ModuleList([self.fg for _ in range(config.encoder_layers)])
        # self.final_layer_norm = nn.LayerNorm(embed_dim)
        # self.sigmiod = nn.Sigmoid()
        # # self.img_feature_transfers = nn.ModuleList([self.img_feature_transfer for _ in range(config.encoder_layers)])
        # # self.text_feature_transfer = nn.ModuleList([self.text_feature_transfer for _ in range(config.encoder_layers)])
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig,
        ...
        self.encoder = BartEncoder(config, self.shared,
                                   fusion_layer, use_img_trans, use_forget_gate, cross_attn_type, dim_common, n_attn_heads, visual_hidden_size)
...
class BartForMultiModalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, fusion_layer=None, use_img_trans=None, use_forget_gate=None, cross_attn_type=None, dim_common=256, n_attn_heads=1, visual_hidden_size=2048):
        super().__init__(config)
        self.model = BartModel(config, fusion_layer, use_img_trans, use_forget_gate, cross_attn_type, dim_common, n_attn_heads, visual_hidden_size)
