...
from transformers import CLIPProcessor  # NEW: For CLIP support
from tqdm import tqdm
import pytorch_lightning as pl
import torch
import numpy as np
from utils.utils import pad_sents, get_mask
from PIL import Image  # NEW: For image loading
import cv2  # NEW: For video loading
...
class OurDataset(Dataset):
    ...
    def __init__(self, args, mode):
        ...
        # Read visual dimension from args (works for all VL models)
        self.visual_hidden_size = getattr(args, 'visual_hidden_size', 2048)
        print(f"[{mode}] Using pre-extracted features ({self.visual_hidden_size}-dim)")
...
#         # NEW: Initialize CLIP processor if using CLIP
#         self.use_clip = getattr(args, 'use_clip', False)
#         if self.use_clip:
#             print(f"[{mode}] Using CLIP features (512-dim)")
#             self.visual_hidden_size = getattr(args, 'visual_hidden_size', 512)
#         else:
#             print(f"[{mode}] Using pre-extracted features (2048-dim)")
#             self.visual_hidden_size = 2048
...
#         self.tgt_ids = self.tokenize(self.tgt)
    ...
    def collate_fn(self, data):
        if self.args.model == 'text_only_bart':
            ...
            src = []
            tgt = []
            # MODIFIED: Support both CLIP (512) and ResNet (2048) features
            img = np.zeros([len(raw_src), self.args.max_img_len, self.visual_hidden_size])
            img_len = []
            # remove blank data
            for i in range(len(raw_src)):
                src.append(raw_src[i])
